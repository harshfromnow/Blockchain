...
def find_nonce(block_hash):
    ...
    while True:
        ...
        computed_hash = _sha3_256(data)
        if computed_hash[-1] == '0':
            return nonce
        nonce += 1

def find_miner(miners, block_number):
    block_index = block_number % 8
    h=0
    for k in miners:
        score = int(k[block_index+2])*int(k[1])
        if  score >= h:
            h = score
            m = k[0]
    return m
...
def main():
    ...
    for txn in txns:
        ...
        #4 transactions
        if len(current_block_txns) == 4 or txn == txns[-1]:
            #Merkle_root
            ...
            #nonce
            nonce = find_nonce(block_hash)
            #miner
            miner = find_miner(miners, block_number)
            #increasing balance
            balances[miner] += block_reward
            #Output
            print(block_number)
            print(block_hash)
            print(current_block_txns)
            print(merkle_root)
            print(str(nonce)+' '+miner)
